...
if($_SESSION['is_admin'] == true){
    ...
    $bookings_data = $selectBooking->fetchAll();
        ...
        $sql = "SELECT movies.movie_name, users.email, booking.id, booking.nr_tickets, booking.date, booking.time, FROM movies
        INNER JOIN bookings ON movies.id = bookings.movie_id
        INNER JOIN users ON users.id = booking.user_id WHERE booking.user_id = :user_id";

    $selectBooking = $conn->prepare($sql);
    $selectBooking->bindParam(":user_id", $user_id);
    $selectBooking->execute();

    $booking_data = $selectBooking->fetchAll();
